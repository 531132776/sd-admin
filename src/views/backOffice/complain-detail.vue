    ...
    <div class="complain-detail">
        ...
        <div class="credentials">
            ...
            <div class="upload-conrtol">
                <!-- <el-upload name="submitFile" :file-list="complainProofPicList" action="/api/pc/file/upload" list-type="picture-card"
    ...
    export default {
        ...
        mounted(){
            if(this.$route.query&&this.$route.query.type ){
                this.postUrl = '/api/pc/finance/complaint/order/update';
                this.routeName = 'financeComplain';
            }else{
                this.postUrl = '/api/pc/internal/complaint/order/update';
                this.routeName = 'internalComplain';
            }
        },
        methods: {
            setTrue() {
                this.$prompt(this.$t('Pleaseentertheremark'), this.$t('prompt'), {
                    ...
                    this.$axios.post(this.postUrl, this.$qs.stringify({ id: this.detail.id,isVerified:1,remark: value }))
                        .then(res => this.$router.push({ name: this.routeName }))
            ...
            setFalse() {
                this.$prompt(this.$t('Pleaseentertheremark'), this.$t('prompt'), {
                    ...
                    let loadingInstance = this.$Loading.service({ fullscreen: true, text: 'loading', spinner: 'el-icon-loading', background: 'rgba(56, 53, 53, 0.6)' });
                    this.$axios.post(this.postUrl, this.$qs.stringify({ id: this.detail.id,isVerified:2, remark: value }))
                        .then(res => this.$router.push({ name: this.routeName }))
                        .catch(err => this.$message.error(err.message))
                        .finally(() => this.$nextTick(() => {
                            loadingInstance.close();
