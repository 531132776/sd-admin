        ...
        <el-table class="tablelist_close" :fit="true"
            ...
            <el-table-column prop="nickname" :label="$t('operation')" header-align="center" align="center" >
                <template slot-scope="scope">
                    ...
                    <a class="ml-10" v-if="scope.row.contractId && scope.row.standby2 && $route.params.type ==1" :href="scope.row.standby2" target="blank" :style="{'color':'#333333'}">{{$t('preview')}}</a>
                </template>
            </el-table-column>
...
export default {
    ...
    methods:{
        ...
        view(item){
            ...
            if(item.contractId && item.standby2 && this.$route.params.type==1){
